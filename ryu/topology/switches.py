...
class LLDPPacket(object):
    ...
    def lldp_packet(dpid, port_no, dl_addr, ttl, timestamp,
         ...
         tlv_chassis_id = lldp.ChassisID(subtype=lldp.ChassisID.SUB_LOCALLY_ASSIGNED,chassis_id=LLDPPacket.CHASSIS_ID_FMT % dpid_to_str(dpid))
...
class Switches(app_manager.RyuApp):
    ...
    def _port_added(self, port):
        _time=time.time()
        lldp_data = LLDPPacket.lldp_packet(port.dpid, port.port_no, port.hw_addr, self.DEFAULT_TTL,_time)
